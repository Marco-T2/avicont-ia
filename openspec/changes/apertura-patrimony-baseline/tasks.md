...
- [ ] T10 RED: write failing builder test — `aperturaBaseline` omitted (field absent) → existing behavior unchanged, CA NOT absorbed, `imbalanced === true` if delta is unaccounted (REQ-3 MODIFIED scenario "CA sin aperturaBaseline mantiene comportamiento previo")
