# Spec: Módulo de Auditoría

**Change**: `modulo-de-auditoria`
**Status**: Proposed
**Date**: 2026-04-24
**Depends on**: Proposal `modulo-de-auditoria`, spec `feature-module-boundaries`, spec `rbac-permissions-matrix`

## Scope

Este spec formaliza, en términos verificables, la ruta de **lectura** sobre `audit_logs` expuesta a administradores: dos endpoints (lista agrupada por comprobante y detalle por comprobante), un classifier cerrado directa/indirecta, blindajes de tenant isolation locales al módulo, el alta del resource `"audit"` en la matriz RBAC, una data migration idempotente que propaga el permiso a orgs existentes, dos índices nuevos en `audit_logs`, y el diff viewer con whitelist por `entityType`. El feature folder `features/audit/` nace cumpliendo el spec `feature-module-boundaries` (split-native).

Trazabilidad: cada REQ de esta sección mapea uno-a-uno a un Acceptance criterion del proposal (ver tabla al final).
