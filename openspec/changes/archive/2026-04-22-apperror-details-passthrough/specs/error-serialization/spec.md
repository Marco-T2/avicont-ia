# Delta Spec: error-serialization

**Capability:** `error-serialization`
**Change:** `apperror-details-passthrough`
**Date:** 2026-04-22
**Phase:** Spec
**Type:** Delta spec — new capability (transversal infrastructure)

---

## Scope

This is a delta spec for a new, standalone capability: the HTTP error serialization contract enforced by `features/shared/middleware.ts handleError`. It does NOT modify any existing canonical spec. The rules below are forward contracts that the apply phase will implement and all future contributors must honor.
...
- This delta spec covers only the `handleError` serialization layer. It does NOT define new business rules for any specific domain.
- This spec does NOT modify `openspec/specs/monthly-period-close/spec.md` or any other canonical spec. REQ-4 of that spec already mandates the structured response; this spec documents how the middleware delivers it.
- The `ZodError` and unknown-error branches are referenced here as regression guards only — their behavior is established and must remain stable.
- UI consumption patterns (e.g., refactoring `monthly-close-panel.tsx` to read `error.details` directly) are out of scope for this change and owned by `monthly-close-ui-reconciliation`.
- REQ-6 governs the module structure of the serializer; it does NOT require removing all `vi.mock` calls in all test files — only the aspirational mock that was masking an unimplemented contract (AC7).
