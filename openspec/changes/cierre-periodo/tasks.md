...
<!-- VERIFY -->
- [ ] T06 Run Phase 1 suite — `pnpm vitest run features/monthly-close/__tests__/migration.smoke.test.ts features/monthly-close/__tests__/audit-trigger.test.ts`. Acceptance: T01–T05 all green; migration applies cleanly on test DB; existing tests in other suites remain green (no regressions from CASCADE drop).
...
<!-- GREEN -->
- [ ] T11 Add `countDraftDocuments(organizationId, periodId)` to repository — touches `features/monthly-close/monthly-close.repository.ts`. Acceptance: method runs three parallel `count` queries for Dispatch/Payment/JournalEntry with `status = 'DRAFT'`; returns `{ dispatches, payments, journalEntries }`; makes T10 pass.
...
<!-- GREEN -->
- [ ] T17 Update `closePeriod`/add `markPeriodClosed` in repository — touches `features/monthly-close/monthly-close.repository.ts`. Acceptance: replaces existing `closePeriod` (or renames it to `markPeriodClosed`) to accept `userId: string`; sets `status: 'CLOSED'`, `closedAt: new Date()`, `closedBy: userId`; makes T16 pass.

<!-- VERIFY -->
- [ ] T18 Run Phase 3 repository suite — `pnpm vitest run features/monthly-close/__tests__/monthly-close.repository.test.ts`. Acceptance: all repository tests (T10–T17) green; real Postgres test DB used.
...
<!-- GREEN -->
- [ ] T22 Create `monthly-close.validation.ts` — touches `features/monthly-close/monthly-close.validation.ts`. Acceptance: exports `closeRequestSchema` as `z.object({ periodId: z.string().min(1), justification: z.string().optional() })`; makes T21 pass.

<!-- VERIFY -->
- [ ] T23 Run Phase 4 suite — `pnpm vitest run features/monthly-close/__tests__/monthly-close.types.test.ts features/monthly-close/__tests__/monthly-close.validation.test.ts`. Acceptance: all type and validation tests green.
...
<!-- RED -->
- [ ] T26 Write failing unit test — `PERIOD_HAS_DRAFTS` — touches `features/monthly-close/__tests__/monthly-close.service.test.ts`. Acceptance: `it('close throws PERIOD_HAS_DRAFTS with per-entity counts')` — mocked `countDraftDocuments` returns `{ dispatches: 2, payments: 0, journalEntries: 1 }`; asserts error code `PERIOD_HAS_DRAFTS` and error payload includes `{ dispatches: 2, journalEntries: 1 }`; fails because service does not call `countDraftDocuments` (new method name).
...
<!-- GREEN -->
- [ ] T35 Add `period` resource to permissions + update route RBAC — touches `features/shared/permissions.ts`, `app/api/organizations/[orgSlug]/monthly-close/route.ts`. Acceptance: adds `"period"` to the `Resource` union type; adds `period` to `PERMISSIONS_READ` and `PERMISSIONS_WRITE` maps (gated to `['owner', 'admin']` for write as default — `'period:close'` semantics are write-level); updates route to call `requirePermission('period', 'close', orgSlug)` — since `Action` is `'read' | 'write'`, the new action name must be reconciled (see Open Questions OQ-1 below — use `'write'` for now under resource `'period'`); makes T34 pass.

<!-- RED -->
- [ ] T36 Write failing route handler test — full dispatch + error codes — touches `app/api/organizations/[orgSlug]/monthly-close/__tests__/route.test.ts`. Acceptance: six `it` blocks: (a) `'POST returns 200 with CloseResult on success'` — mocked service returns valid result; (b) `'POST returns 409 PERIOD_ALREADY_CLOSED'`; (c) `'POST returns 422 PERIOD_UNBALANCED'`; (d) `'POST returns 422 PERIOD_HAS_DRAFTS'`; (e) `'POST returns 404 PERIOD_NOT_FOUND'`; (f) `'POST returns 400 on invalid payload (missing periodId)'` — Zod validation fires; all fail because route does not validate body with Zod schema and does not propagate `justification`.

<!-- GREEN -->
- [ ] T37 Update route handler — Zod validation + justification + new service signature — touches `app/api/organizations/[orgSlug]/monthly-close/route.ts`. Acceptance: parses body with `closeRequestSchema`; resolves `user.id`; calls `service.close({ organizationId: orgId, periodId, userId: user.id, justification })`; `handleError` maps `ConflictError` → 409, `NotFoundError` → 404, `ValidationError` → 422, `ForbiddenError` → 403; makes T36 pass.

<!-- VERIFY -->
- [ ] T38 Run Phase 6 suite — `pnpm vitest run features/monthly-close/__tests__/monthly-close.rbac.test.ts app/api/organizations/[orgSlug]/monthly-close/__tests__/route.test.ts`. Acceptance: all RBAC and route handler tests green.
...
<!-- GREEN -->
- [ ] T55 Migrate `PeriodCloseDialog` to canonical endpoint — touches `components/accounting/period-close-dialog.tsx`. Acceptance: changes fetch target to `POST /api/organizations/${orgSlug}/monthly-close`; body includes `{ periodId: period.id, justification? }`; adds optional `<Textarea>` for justification input in the dialog; makes T54 pass.

<!-- VERIFY -->
- [ ] T56 Run Phase 8 suite — `pnpm vitest run app/api/organizations/[orgSlug]/periods/[periodId]/__tests__/route.test.ts components/accounting/__tests__/period-close-dialog.test.tsx`. Acceptance: 410 Gone on close PATCH + GET still 200 + dialog calls new endpoint — all green.
...
<!-- GREEN -->
- [ ] T58 Update `MonthlyClosePanel` with justification textarea and balance display — touches `components/settings/monthly-close-panel.tsx`. Acceptance: adds justification `<Textarea>` in confirmation dialog; passes `justification` in POST body; renders DEBE≠HABER warning banner if summary indicates imbalance (requires summary endpoint to return balance info — see OQ-2); makes T57 pass.
...
<!-- VERIFY -->
- [ ] T62 Run full vitest suite — `pnpm vitest run`. Acceptance: all tests green; no regressions in existing dispatch, payment, journal, sale, purchase, or fiscal-period suites; `PERIOD_HAS_DRAFT_ENTRIES` vs `PERIOD_HAS_DRAFTS` constant naming resolved without breaking prior tests.
...
**OQ-1 — `period:close` vs `period:write`**: `requirePermission` accepts `Resource` and `Action: 'read' | 'write'`. The spec requires a new `period:close` permission. The action string `'close'` does not fit `Action = 'read' | 'write'`. Two options: (a) add `'close'` to the `Action` union (widens the type, affects all route call sites — low risk since `requirePermission` is the only consumer of `Action`); (b) model it as `requirePermission('period', 'write', ...)` with `period` as a new `Resource` (uses existing `Action` union, simpler). Option (b) was adopted in T35 as the lowest-friction path. If the team decides option (a) is preferable, T35 must be updated to add `'close'` to `Action` and update the PERMISSIONS matrix accordingly.

**OQ-2 — Summary endpoint balance data**: The `MonthlyCloseSummary` type and `/monthly-close/summary` route currently do not return `debit`/`credit` totals. T58 (UI balance warning) depends on this data. Either: (a) extend the summary endpoint to call `sumDebitCredit` and return `{ balanced: boolean, debit: string, credit: string, diff: string }`; or (b) omit the balance warning from the UI panel (it is shown only at close time as an error, not pre-emptively). Recommend option (a) for better UX — add a T58a GREEN task to extend the summary route if chosen. This did not block the phase ordering since T59/T60 are independent.

**OQ-3 — `PERIOD_HAS_DRAFT_ENTRIES` vs `PERIOD_HAS_DRAFTS` constant naming**: `errors.ts` already exports `PERIOD_HAS_DRAFT_ENTRIES`. The spec uses `PERIOD_HAS_DRAFTS`. T20 adds `PERIOD_HAS_DRAFTS`; if the existing constant is referenced by existing tests, both must coexist briefly (or the existing constant is aliased). Verify during T20 that no existing test breaks.
