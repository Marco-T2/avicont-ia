...
**Capability**: `journal-entry-form-ux`
**First published**: 2026-04-23 (from change `journal-form-date-aware-period`)
**Type**: Canonical spec
...
Auto-selection MUST re-run on every `date` change UNLESS the user has manually overridden the period selector.

Once the user manually interacts with the period `<Select>`, subsequent `date` changes MUST NOT overwrite `periodId`.
...
- **AC-1.4 omitted**: "form reset clears dirty-flag" was a scenario in the delta spec but has no implementation surface — the form has no reset handler (`grep -n 'reset\|Reset\|onReset\|form.reset'` in `journal-entry-form.tsx` returns zero matches). Removed at canonical promotion to keep spec and code in honest alignment. If a reset action is added later, wire `setPeriodManuallySelected(false)` and reopen this contract explicitly.
