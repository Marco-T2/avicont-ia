...
The audit log infrastructure captures every significant mutation on financial entities into the `audit_logs` table via PostgreSQL triggers and session variables. This spec documents the canonical contract for correlation-ID propagation, triggered tables, and service-layer enforcement of justification on LOCKED document edits.

The base infrastructure (pre-existing, not re-specified in detail here) provides:
...
- Trigger on `sales`: AFTER UPDATE OR DELETE, FOR EACH ROW, invoking `audit_trigger_fn()`.
- `setAuditContext(tx, userId, justification?)`: sets `app.current_user_id` and `app.audit_justification` via `SET LOCAL`.

The requirements below define the canonical behavior layered on top of that base.
