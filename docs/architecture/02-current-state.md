...
|---|---|---|---|---|---|
| `features/accounting` | ~12,252 | 65 | 128 | OLEADA 6 COMPLETE — shim retirement CLOSED | **SHIM RETIREMENT COMPLETE (OLEADA 6 sub-POC 8/8)**: 3 shim files DELETED (`journal.service.ts`, `ledger.service.ts`, `accounting.validation.ts` re-export). `server.ts` barrel SLIMMED: removed 3 shim re-exports; RETAINED `JournalRepository` + `journal.types` + `parseEntryDate` + `AutoEntryGenerator` + `accounting-helpers` + `document-lifecycle` (TrimPreviewItem/validateLockedEdit) + `correlative.utils`. All 26 barrel consumers repointed to hex factories `makeJournalsService()`/`makeLedgerService()` at `@/modules/accounting/presentation/server`. `journal.repository.ts` KEPT (invariant collision — runtime adapter + tests). Canonical homes for journal/ledger services = hex factory. Previous stages: 3 type files → SHIM (POC #2a); 4 utils files → SHIM (POC #2b); 2 account-subtype files → SHIM (POC #2c); 1 ui-helper file → SHIM (POC #2d). **poc-accounting-journal-ledger-core-hex (sub-POC 7/8)**: journal/ledger CORE folded into hex (C0–C5); legacy services became thin shims (class names + sigs preserved, ZERO impl logic, lazy cycle-break). **DEV-1/R-money**: float money-math preserved in hex reads/LedgerService/auto-entry (NOT converged to shared Decimal — deferred follow-up). **OLEADA 6 FULLY COMPLETE 8/8** (trial-balance + equity-statement + worksheet + initial-balance + iva-books + exporters-cleanup + journal-ledger-core + shim-retirement) |
