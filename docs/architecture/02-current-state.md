...
> **Cementación**: POC docs-refactor recon inventory cumulative cross-POC matures. Updated POC #3f 2026-05-12. Updated poc-ai-agent-hex C5 OLEADA 5 2/3 CLOSED 2026-05-13. Updated poc-financial-statements-hex C5 OLEADA 5 FULLY CLOSED 3/3 2026-05-13. Updated poc-accounting-exporters-cleanup C2 OLEADA 6 sub-POC 6/8 CLOSED 2026-05-14. Updated poc-accounting-journal-ledger-core-hex D1 OLEADA 6 sub-POC 7/8 CLOSED 2026-05-14.
...
|---|---|---|---|---|---|
| `features/accounting` | ~12,252 | 65 | 128 | 5 — residual POST-POC #2d shim; **journal/ledger core FOLDED OLEADA 6 sub-POC 7/8** | 3 type files → SHIM (POC #2a); 4 utils files → SHIM (POC #2b: account-code, correlative, accounting-helpers, journal.dates); 2 account-subtype files → SHIM (POC #2c: utils + resolve); 1 ui-helper file → SHIM (POC #2d: journal.ui, −41 net LOC); canonical homes `/modules/accounting/presentation/dto/` + `/modules/accounting/domain/`. **poc-accounting-journal-ledger-core-hex CLOSED**: journal/ledger CORE folded into the hex — `journal.service.ts` (661→182 LOC) + `ledger.service.ts` (113→63 LOC) are now thin delegating shims over the hex (class names + public sigs preserved, ZERO impl logic, lazy `import()` cycle break); `journal.repository.ts` KEPT; `auto-entry-generator.ts` + `document-lifecycle.service.ts` + `exporters/` git-mv'd into the hex; `accounting.validation.ts` → thin `export *` re-export shim. The `server.ts` barrel + 26 barrel consumers DELIBERATELY UNTOUCHED — shim retirement is sub-POC 8 (`poc-accounting-shim-retirement`). DEV-1/R-money: float money-math preserved in the folded reads/LedgerService/auto-entry, NOT converged to shared Decimal money.utils (deferred follow-up) |
