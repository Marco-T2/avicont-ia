...
/**
 * Unit tests para mapper hex domain entity → legacy DTO purchase (POC
 * #11.0c A4-c C2 GREEN P3 sub-locks Marco). Mirror simétrico sale mapper
 * tests salvo asimetrías declaradas (purchaseId, nitProveedor, tipoCompra,
 * NO estadoSIN). 5 tests cubren las 4 conversion types + batch wrapper.
 */
...
function makeFixture(overrides: Partial<{
  ...
  const entry = IvaPurchaseBookEntry.create({
    ...
    purchaseId:
      overrides.purchaseId === null
        ? undefined
        : overrides.purchaseId ?? "purchase-test",
    ...
    tipoCompra: overrides.tipoCompra ?? 1,
...
describe("entity-to-dto purchases mapper — POC #11.0c A4-c C2 GREEN P3 lockeada", () => {
  it("per-row identity: preserves 30 fields shape mapping (id, status, fechaFactura, nitProveedor, razonSocial, tipoCompra, etc.)", () => {
    const entry = makeFixture({ tipoCompra: 2 });
    ...
    expect(dto.nitProveedor).toBe(entry.nitProveedor);
  ...
  it("batch wrapper entriesToDto preserves order + applies entityToDto per-row + purchaseId/notes null→undefined coalesce", () => {
    ...
    expect(dtos[0].purchaseId).toBeUndefined();
    expect(dtos[0].notes).toBeUndefined();
    expect(dtos[1].purchaseId).toBe("purchase-explicit");
