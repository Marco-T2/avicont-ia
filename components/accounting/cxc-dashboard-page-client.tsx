...
// `contact-ledger-page-client` shadow types — JSON.parse(JSON.stringify(...))
// in the RSC downgrades any Date to string, balances already serialize as
// strings server-side.
...
interface CxcDashboardPageClientProps {
  orgSlug: string;
...
/**
 * Builds the dashboard URL preserving / toggling the relevant query params.
 * `contactId` does NOT live in the URL on this surface (it's a per-row link
 * to the detail page). Only filters (includeZeroBalance, sort, direction,
 * page) round-trip.
 */
function buildDashboardHref(
  ...
  // Sort/direction: include when explicitly present in `patch` OR `current`
  // — preserves user choice across navigations even when value matches
  // the server-side default (default-coalescing happens at the route /
  // service layer; the URL should round-trip the user's explicit selection).
...
export default function CxcDashboardPageClient({
  ...
  function handleToggleZero(e: React.ChangeEvent<HTMLInputElement>) {
    // checkbox "Solo con saldo" is the inverse of includeZeroBalance —
    // unchecking means "Mostrar todos" => includeZeroBalance=true.
