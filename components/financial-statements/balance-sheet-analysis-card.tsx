...
export type AnalyzeBalanceSheetResult =
  | { status: "ok"; analysis: string }
  | { status: "trivial"; code: string; reason: string }
  | { status: "error"; reason: string };

interface BalanceSheetAnalysisCardProps {
  result: AnalyzeBalanceSheetResult | null;
  loading: boolean;
}
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          Análisis IA del Balance General
        </CardTitle>
      </CardHeader>
      <CardContent>
        {loading && (
