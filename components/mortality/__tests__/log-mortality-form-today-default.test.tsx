 ...
 * RED → GREEN
 *
 * C-1 fix — log-mortality-form.tsx date default uses todayLocal(),
...
import LogMortalityForm from "../log-mortality-form";
...
function openDialog() {
  render(<LogMortalityForm orgSlug="test-org" lotId="lot-1" aliveCount={1000} />);
  fireEvent.click(screen.getByRole("button", { name: /registrar mortalidad/i }));
  return document.querySelectorAll<HTMLInputElement>('input[type="date"]');
}

// ── Tests ──

describe("LogMortalityForm — date default (C-1)", () => {
  beforeAll(() => {
    vi.useFakeTimers();
  });
