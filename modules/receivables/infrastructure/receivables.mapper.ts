...
import { OverdueReceivableNotPersistable } from "../domain/errors/receivable-errors";
 ...
 * OverdueReceivableNotPersistable, a ValidationError that `handleError`
 ...
 * (accounts_receivable_status_no_overdue_check) is not yet applied — editing
...
export function assertPersistableStatus(status: ReceivableStatus): void {
  if (status === "OVERDUE") {
    throw new OverdueReceivableNotPersistable();
  }
}
