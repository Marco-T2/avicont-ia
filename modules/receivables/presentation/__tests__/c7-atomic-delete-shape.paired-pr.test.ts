/**
 * POC paired payables↔receivables C7 — atomic delete features/receivables/
 * wholesale shape (cxc side, paired sister mirror payables).
 *
 * Axis: legacy `features/receivables/` directory wholesale deletion (6 source
 * files — server.ts shim @deprecated + index.ts re-export barrel +
 * receivables.types.ts legacy types module + receivables.validation.ts
 * pass-through re-export hex canonical + receivables.service.ts legacy shim
 * DTO bridge + receivables.repository.ts legacy alias) post cumulative cutover
 * hex `modules/receivables` completado:
 *   - C0 dispatch cleanup absorb residuals
 *   - C1a service hex factory `makeReceivablesService()`
 ...
 *   - C5-C6 drop legacy POJO type defs `ReceivableWithContact` + `OpenAggregate`
 ...
 *
 * Cero CONSUMER PRODUCCIÓN residual `@/features/receivables(/server)` verified
 ...
 * TEST-MOCK-DECLARATION legacy 0 + RESIDUAL solo features/receivables/* internos
 ...
 * HEX `Receivable` entity + factory `makeReceivablesService()` + bridge
 * `attachContact[s]` resolved via `@/modules/receivables/presentation/server`
 ...
 *
 *   ── Tests 1-6 FAIL (transición RED → GREEN, paired side cxc) ──
 *   - Test 1 FAIL: `features/receivables/server.ts` EXISTS pre-GREEN
 *     (`fs.existsSync === true`, NO false). GREEN C7 sub-paso 1 deletes
 *     archivo → assertion transitions RED → GREEN.
 *   - Test 2 FAIL: `features/receivables/index.ts` EXISTS pre-GREEN. GREEN
 *     C7 sub-paso 2 deletes archivo → transition.
 *   - Test 3 FAIL: `features/receivables/receivables.types.ts` EXISTS
 *     pre-GREEN. GREEN C7 sub-paso 3 deletes archivo → transition.
 *   - Test 4 FAIL: `features/receivables/receivables.validation.ts` EXISTS
 *     pre-GREEN (pass-through re-export preserved C7-pre scope, drop C7
 *     wholesale). GREEN C7 sub-paso 4 deletes archivo → transition.
 *   - Test 5 FAIL: `features/receivables/receivables.service.ts` EXISTS
 *     pre-GREEN (legacy shim DTO bridge preserved C5-C6 scope post-swap a
 *     hex types, drop C7 wholesale). GREEN C7 sub-paso 5 deletes archivo
 *     → transition.
 *   - Test 6 FAIL: `features/receivables/receivables.repository.ts` EXISTS
 *     pre-GREEN (legacy alias `ReceivablesRepository = PrismaReceivablesRepository`).
 ...
 *     `from "@/features/receivables/server"` ALREADY verified retirement
 ...
 *     estricto).
 *   - Test 8 PASS pre-GREEN: idem barrel exact `from "@/features/receivables"`
 ...
 *     declarations dead).
 *
 * Total expected pre-GREEN cxc side: 6 FAIL (Tests 1-6) + 2 PASS (Tests 7-8 safety net).
 ...
 * `47d7bfb` body Read confirmation pre-RED C7). Paired sister cxp side
 ...
 * inicio): este shape file vive en `modules/receivables/presentation/__tests__/`
 * (NO bajo `features/receivables/__tests__/` que NO existe). Pattern
 ...
 * - modules/receivables/presentation/__tests__/c0-dispatch-cleanup-shape.paired
 *   -pr.test.ts (precedent C0 RED — paired sister)
 * - modules/receivables/presentation/__tests__/c1b-attach-contact-boundary-shape
 ...
 * - modules/receivables/presentation/__tests__/c5-c6-dto-drop-shape.paired-pr.test.ts
 *   (precedent C5-C6 RED — paired sister 26 assertions α 13 per side)
 * - modules/receivables/presentation/__tests__/c7-pre-cutover-schemas-shape.paired
...
// ── C7 RED paths cxc side (6 source file existence transition, dir absorbed) ──

const RECEIVABLES_SERVER_PATH = path.join(
  REPO_ROOT,
  "features/receivables/server.ts",
);
const RECEIVABLES_INDEX_PATH = path.join(
  REPO_ROOT,
  "features/receivables/index.ts",
);
const RECEIVABLES_TYPES_PATH = path.join(
  REPO_ROOT,
  "features/receivables/receivables.types.ts",
);
const RECEIVABLES_VALIDATION_PATH = path.join(
  REPO_ROOT,
  "features/receivables/receivables.validation.ts",
);
const RECEIVABLES_SERVICE_PATH = path.join(
  REPO_ROOT,
  "features/receivables/receivables.service.ts",
);
const RECEIVABLES_REPOSITORY_PATH = path.join(
  REPO_ROOT,
  "features/receivables/receivables.repository.ts",
);
...
const LEGACY_BARREL_IMPORT_RE = /from\s*["']@\/features\/receivables["']/;
...
describe("POC paired payables↔receivables C7 — atomic delete features/receivables/ wholesale shape (cxc side)", () => {
  // ── Tests 1-6: source files no longer exist (legacy wholesale deletion paired sister cxc) ──

  it("Test 1: features/receivables/server.ts no longer exists (legacy shim @deprecated deletion C7 — barrel re-export `ReceivablesService` + `ReceivablesRepository` resolved canonical home `@/modules/receivables/presentation/server` post-cutover cumulative)", () => {
    expect(fs.existsSync(RECEIVABLES_SERVER_PATH)).toBe(false);
  });

  it("Test 2: features/receivables/index.ts no longer exists (legacy re-export barrel deletion C7 — schemas zod + types absorbed canonical home post C7-pre cutover)", () => {
    expect(fs.existsSync(RECEIVABLES_INDEX_PATH)).toBe(false);
  });

  it("Test 3: features/receivables/receivables.types.ts no longer exists (legacy types module deletion C7 — POJO `ReceivableWithContact` + `OpenAggregate` already dropped C5-C6 wholesale, drop residual `Prisma` re-exports C7)", () => {
    expect(fs.existsSync(RECEIVABLES_TYPES_PATH)).toBe(false);
  });

  it("Test 4: features/receivables/receivables.validation.ts no longer exists (legacy pass-through re-export deletion C7 — schemas zod canonical home `@/modules/receivables/presentation/validation` consumed direct post C7-pre cutover)", () => {
    expect(fs.existsSync(RECEIVABLES_VALIDATION_PATH)).toBe(false);
  });

  it("Test 5: features/receivables/receivables.service.ts no longer exists (legacy shim DTO bridge deletion C7 — `ReceivablesService` factory hex `makeReceivablesService()` consumed via composition-root post-cutover, return types swap a `ReceivableSnapshotWithContact` C5-C6 absorbed)", () => {
    expect(fs.existsSync(RECEIVABLES_SERVICE_PATH)).toBe(false);
  });

  it("Test 6: features/receivables/receivables.repository.ts no longer exists (legacy alias `ReceivablesRepository = PrismaReceivablesRepository` deletion C7 — canonical home `@/modules/receivables/presentation/server` consumed direct post-cutover)", () => {
    expect(fs.existsSync(RECEIVABLES_REPOSITORY_PATH)).toBe(false);
  });
  ...
  it("Test 7: zero production source imports `from \"@/features/receivables/server\"` (PROJECT-scope safety net consumer reintroducción cxc side)", () => {
    const matches = findFilesMatchingImport(LEGACY_SERVER_IMPORT_RE);
    expect(matches).toEqual([]);
  });

  it("Test 8: zero production source imports `from \"@/features/receivables\"` exact barrel (PROJECT-scope safety net consumer reintroducción cxc side)", () => {
