 ...
 * `ReceivableWithContact` consistente shim (pre) ↔ infrastructure/ (post).
 *
...
describe("POC paired payables↔receivables C1b-α — boundary attachContact Option A push INTO infrastructure/contact-attacher.ts shape (paired-receivables side, §13.A5-γ MATERIAL boundary preservation, 7ma evidencia §13.A5-α paired sister sub-cycle functional move NEW kind + R5 invariant collision elevation honored)", () => {
  ...
  it("Test 3: modules/receivables/infrastructure/contact-attacher.ts contains Prisma.Decimal reference (mapper reconstruction MonetaryAmount.value→Prisma.Decimal at infrastructure layer per Marco lock L3 + CR1 α-A3 R5 honor)", () => {
