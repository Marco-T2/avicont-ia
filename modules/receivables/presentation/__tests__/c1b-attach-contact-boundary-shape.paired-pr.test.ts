 ...
 * push INTO infrastructure/contact-attacher.ts shape (paired-receivables side,
 * paired sister mirror payables).
 *
 ...
 *
 * Axis: functional move 3 privados shim INTO modules/receivables/infrastructure/
 ...
 *     → modules/receivables/infrastructure/contact-attacher.ts (NEW file)
 ...
 *     → modules/receivables/infrastructure/contact-attacher.ts (NEW file)
 *   - features/receivables/receivables.service.ts L124-146: toReceivableSnapshotWithContact
 ...
 *   - modules/receivables/presentation/server.ts ADDS:
 ...
 *
 * Marco lock final RED-α scope (4 assertions α paired-receivables side — paired
 * sister mirror payables 4 assertions = 8 paired total parsimonious vs C1a 10):
 ...
 *     POJO contract `ReceivableSnapshotWithContact = AccountsReceivable & { contact: Contact }`
 ...
 *     `Promise<ReceivableSnapshotWithContact>` PRESERVED — C1b internal refactor only,
 ...
 * `ReceivableSnapshotWithContact` consistente shim (pre) ↔ infrastructure/ (post).
 *
 ...
 *   - T1 FAIL: server.ts barrel hoy 54 LOC pure barrel re-export, NO contiene
 *     `attachContact` literal. Regex match falla.
 *   - T2 FAIL: server.ts NO contiene `attachContacts` literal. Regex match falla.
 *   - T3 FAIL: modules/receivables/infrastructure/contact-attacher.ts NO existe
 ...
 * asserta paths `modules/receivables/presentation/server.ts` +
 * `modules/receivables/infrastructure/contact-attacher.ts` que persisten post C7
 * wholesale delete `features/receivables/`. Test vive en
 * modules/receivables/presentation/__tests__/ — NO toca features/receivables/* que
 ...
 *   - features/receivables/receivables.types.ts (ReceivableSnapshotWithContact = AccountsReceivable & { contact: Contact } legacy POJO contract preserved)
 *   - modules/receivables/presentation/server.ts (target boundary hex barrel — barrel re-export pre-RED 54 LOC, post-GREEN +1 re-export line)
 *   - modules/receivables/infrastructure/contact-attacher.ts (target NEW file pre-GREEN ENOENT, post-GREEN ~50 LOC functional code at infrastructure/ layer R5 honored)
 *   - modules/receivables/presentation/composition-root.ts (factories pre-existing post C1a closure)
 *   - modules/payables/presentation/__tests__/c1b-attach-contact-boundary-shape.paired-pr.test.ts (paired sister mirror RED-α this batch)
...
// ── C1b-α boundary hex targets (2 archivos paired-receivables side post-CR1 α-A3) ──

const RECEIVABLES_SERVER = path.join(
  REPO_ROOT,
  "modules/receivables/presentation/server.ts",
);
const RECEIVABLES_CONTACT_ATTACHER = path.join(
  REPO_ROOT,
...
describe("POC paired payables↔receivables C1b-α — boundary attachContact Option A push INTO infrastructure/contact-attacher.ts shape (paired-receivables side, §13.A5-γ MATERIAL boundary preservation, 7ma evidencia §13.A5-α paired sister sub-cycle functional move NEW kind + R5 invariant collision elevation honored)", () => {
  // ── Hex boundary barrel re-export POSITIVE (Tests 1-2) ──────────────────

  it("Test 1: modules/receivables/presentation/server.ts re-exports attachContact (singular, barrel re-export from ../infrastructure/contact-attacher per Marco lock CR1 α-A3)", () => {
    const source = fs.readFileSync(RECEIVABLES_SERVER, "utf8");
    expect(source).toMatch(ATTACH_CONTACT_EXPORT_RE);
  });

  it("Test 2: modules/receivables/presentation/server.ts re-exports attachContacts (plural, barrel re-export idem)", () => {
    const source = fs.readFileSync(RECEIVABLES_SERVER, "utf8");
  ...
  it("Test 3: modules/receivables/infrastructure/contact-attacher.ts does NOT contain Prisma.Decimal reference (post-C5-C6 §13.B-paired DTO drop axis simplification — drop reconstruction mapper interno simplifies, .toSnapshot() Path α direct entity → snapshot mapping mirror Opción C precedent A5-C1 — invariant collision elevation R5 absorbed cascade DENTRO C5-C6 lock)", () => {
    const source = fs.readFileSync(RECEIVABLES_CONTACT_ATTACHER, "utf8");
    expect(source).not.toMatch(PRISMA_DECIMAL_RE);
  });

  it("Test 4: modules/receivables/infrastructure/contact-attacher.ts contains prisma.contact.find reference (cross-module direct contact table query at infrastructure layer per Marco lock L4 F3-C + CR1 α-A3 R5 honor — TODO ContactHydratePort extraction future POC contacts)", () => {
    const source = fs.readFileSync(RECEIVABLES_CONTACT_ATTACHER, "utf8");
