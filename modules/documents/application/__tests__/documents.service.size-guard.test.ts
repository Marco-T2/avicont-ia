/**
 * F3 C3.6/C3.7 — 5MB extraction size guard for DOCX (RESOLVED-3).
 *
 * Files >5MB but ≤50MB (outer MAX_FILE_SIZE) MUST NOT crash or block the
 * event loop on sync mammoth parsing. Instead the extractor short-circuits:
 * returns null (no indexed content) and emits a warning log. The Document
 * row is still persisted so the file is downloadable; it's simply absent
 * from RAG search.
 *
 * Worker-thread offload is out-of-scope this SDD (design Risk 1, RESOLVED-3).
 *
 * Scope-locked 2026-05-17: XLSX scenario retired alongside upload format
 * reduction to PDF + DOCX + TXT.
 */
import { describe, it, expect, vi, beforeEach } from "vitest";

const { mockConvertToMarkdown, mockIndexDocument } = vi.hoisted(() => ({
