 ...
 */
import { isSystemRole } from "@/modules/permissions/domain/permissions";
import {
  ValidationError,
  ConflictError,
  RESERVED_SLUG,
...
export function assertNotReserved(slug: string): void {
  if (isSystemRole(slug)) {
    throw new ValidationError(
      `El slug "${slug}" esta reservado por el sistema`,
      RESERVED_SLUG,
    );
  }
}
...
export function resolveUniqueSlug(
  ...
  throw new ConflictError(
    `No se pudo resolver un slug unico para "${base}" (limite ${MAX_COLLISION_SUFFIX})`,
    SLUG_TAKEN,
  );
}
