...
export class MembersService {
  ...
  async addMember(organizationId: string, email: string, role: string) {
    ...
    if (existingUser) {
      ...
      const clerkUsers = await this.clerkAuth.getUsersByEmail(email);
    ...
    if (existing) {
      if (existing.deactivatedAt === null) {
        ...
        throw new ConflictError(
          "El usuario ya es miembro de esta organizacion",
        );
      }
      ...
      if (!reactivateOrg) throw new NotFoundError("Organizacion");
      ...
      const reactivatedDto = await runMemberClerkSaga<{
        ...
        clerkCall: async () => {
          await this.clerkAuth.createOrganizationMembership(
            reactivateOrg.clerkOrgId,
            user.clerkUserId,
            "org:member",
          );
        },
  ...
  async updateRole(
    ...
    // Verificar que el invocador no este cambiando su propio rol (D.4)
  ...
  async removeMember(
    ...
    if (member.user.clerkUserId === currentClerkUserId) {
      throw new ForbiddenError(
        "No podes desactivarte a vos mismo",
        CANNOT_CHANGE_OWN_ROLE,
      );
    }
    ...
    // (per design S1, S11) -- the soft-deleted row would be fine to
    ...
    await runMemberClerkSaga<void>({
      ...
      clerkCall: async () => {
        await this.clerkAuth.deleteOrganizationMembership(
          org.clerkOrgId,
