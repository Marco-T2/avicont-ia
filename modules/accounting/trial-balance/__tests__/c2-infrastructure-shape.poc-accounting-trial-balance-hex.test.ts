 ...
 * pdf.fonts.ts + pdf.helpers.ts were git-mv'd from FS-infra to the shared canonical
 * home @/modules/accounting/shared/infrastructure/exporters/. α46 now asserts the
 * shared path. The remaining FS cross-module ban still holds:
...
describe("POC accounting-trial-balance-hex C2 — infrastructure layer shape", () => {
  ...
  describe("Block 3 — REQ-010 RESOLVED: shared canonical infra import", () => {
    it("α46: trial-balance-pdf.exporter imports from @/modules/accounting/shared/infrastructure/exporters (REQ-010 RESOLVED — shared canonical home, poc-accounting-exporters-cleanup sub-POC 6)", () => {
      const content = readInfraFile("exporters/trial-balance-pdf.exporter.ts");
