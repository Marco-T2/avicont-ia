...
describe("JournalsService.createEntry", () => {
  ...
  // I12 — la fecha del asiento DEBE caer dentro de [period.startDate, period.endDate].
  // Gap previo (verificado Marco UX 2026-05-16): el use case validaba period.status
  // OPEN pero NO coherencia date∈período; el FE auto-derivaba periodId desde la fecha
  // pero un POST directo al API podía mandar {date, periodId} desacoplados. Este test
  // RED demuestra que el guard hoy SÍ rechaza la combinación inválida.
  it("rejects when entry date falls outside the period's [startDate, endDate] (I12)", async () => {
    const { service, accounts, periods, voucherTypes } = setup();
    accounts.accountsById.set("acc-1", {
      id: "acc-1",
      name: "Caja",
    ...
    periods.periodsById.set("period-may-2026", {
      id: "period-may-2026",
      status: "OPEN",
      name: "Mayo 2026",
    ...
    await expect(
      service.createEntry(
        "org-1",
        {
          date: new Date("2026-04-28T12:00:00.000Z"), // April, outside Mayo 2026
          description: "Inconsistencia date↔período",
          periodId: "period-may-2026",
          voucherTypeId: "voucher-1",
          createdById: "user-1",
          lines: [
            { accountId: "acc-1", debit: 100, credit: 0 },
    ...
    ).rejects.toBeInstanceOf(JournalDateOutsidePeriod);
  });
...
describe("JournalsService.updateEntry", () => {
  ...
  // queda en 3/3.
  //
  // Sample sourceType: "sale" (canonical authoritative upstream-bound source).
  // NOT "ai" — AI provenance dissolves on DRAFT→POSTED transition (see
  // sdd/ai-journal-posted-editable). The I9 regression covers sale/purchase/
  // payment/dispatch where provenance persists through post/lock.
