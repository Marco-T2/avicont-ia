...
/**
 * C3 RED — Presentation layer shape tests for POC accounting-worksheet-hex.
 ...
 *
 * D5 INVERSE locked (axis-distinct POSITIVE vs TB):
 * - features/accounting/worksheet/server.ts ALREADY had `import "server-only"` on line 1.
 *   C3 MIGRATES (not adds) — cleanest D5 in OLEADA 6.
 * - NO client.ts — worksheet has ZERO React hooks; pure data module.
 ...
 * - Worksheet: ZERO React hooks confirmed → D5 INVERSE correct (no client.ts).
 *
 ...
 * α65 (walkSources returns empty string for absent MODULE_ROOT files → no "use client" → PASS).
 *
 * REQ mapping (4 blocks / 18α):
 * - Block 1 (α49-α50): composition-root.ts — makeWorksheetService factory + WorksheetService ref
 ...
 *   NO legacy WorksheetRepository class name in server barrel
 *
 * WS-D3 invariant: 1-adapter composition root (PrismaWorksheetRepo → WorksheetService).
 * TB mirror (1 adapter, zero-arg factory) — NOT ES 2-adapter shape.
 *
 ...
 * C0+C1+C2 sentinels verified — none reference presentation/ paths.
 */

const ROOT = path.resolve(__dirname, "../../../..");
const PRESENTATION = path.join(
  ROOT,
  "modules/accounting/worksheet/presentation",
);
const MODULE_ROOT = path.join(ROOT, "modules/accounting/worksheet");
...
function readPresentationFile(relative: string): string {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(
      `ENOENT: cannot read '@/modules/accounting/worksheet/presentation/${relative}'`,
    );
  }
...
describe("POC accounting-worksheet-hex C3 — presentation layer shape", () => {
  // ───────────────────────────────────────────────────────────────────────────
  // Block 1 — composition-root.ts (makeWorksheetService zero-arg factory)
  // WS-D3: 1-adapter, zero-arg factory — EXACT TB mirror (not ES 2-adapter).
  // ───────────────────────────────────────────────────────────────────────────

  describe("Block 1 — composition-root.ts (makeWorksheetService zero-arg, WS-D3)", () => {
    it("α49: makeWorksheetService is exported as a function from presentation/composition-root.ts", () => {
    ...
    it("α50: makeWorksheetService returns WorksheetService (class must be referenced)", () => {
      const content = readPresentationFile("composition-root.ts");
  ...
  // → C3 MIGRATES, not adds. Cleanest D5 OLEADA 6.
  // ───────────────────────────────────────────────────────────────────────────
  ...
  describe('Block 2 — server.ts (REQ-002: `import "server-only"` line 1 + re-exports)', () => {
    it("α51: presentation/server.ts exists", () => {
      ...
      if (!fs.existsSync(filePath)) {
        throw new Error(
          "ENOENT: cannot find '@/modules/accounting/worksheet/presentation/server.ts'",
        );
      }
    ...
    it("α53: server.ts re-exports WorksheetService", () => {
      const content = readPresentationFile("server.ts");
      expect(content).toMatch(/WorksheetService/m);
    });

    it("α54: server.ts re-exports makeWorksheetService from composition-root", () => {
      const content = readPresentationFile("server.ts");
      expect(content).toMatch(/makeWorksheetService/m);
    });

    it("α55: server.ts re-exports worksheetQuerySchema (Zod schema)", () => {
      const content = readPresentationFile("server.ts");
  ...
  // Worksheet types: WorksheetRow, WorksheetTotals, WorksheetReport, WorksheetGroup, WorksheetFilters.
  // NO Serialized* types (unlike TB — worksheet does not have serialization helpers).
  // ───────────────────────────────────────────────────────────────────────────
  ...
  describe("Block 3 — index.ts (client-safe barrel per D5 INVERSE)", () => {
    it("α59: presentation/index.ts exists", () => {
      ...
      if (!fs.existsSync(filePath)) {
        throw new Error(
          "ENOENT: cannot find '@/modules/accounting/worksheet/presentation/index.ts'",
        );
      }
      expect(fs.existsSync(filePath)).toBe(true);
    });

    it("α60: index.ts re-exports core domain types (WorksheetRow, WorksheetTotals, WorksheetReport)", () => {
      const content = readPresentationFile("index.ts");
      expect(content).toMatch(/WorksheetRow/m);
      expect(content).toMatch(/WorksheetTotals/m);
      expect(content).toMatch(/WorksheetReport/m);
    });

    it("α61: index.ts re-exports WorksheetGroup type", () => {
      const content = readPresentationFile("index.ts");
      expect(content).toMatch(/WorksheetGroup/m);
    });

    it("α62: index.ts re-exports WorksheetFilters type", () => {
      const content = readPresentationFile("index.ts");
      expect(content).toMatch(/WorksheetFilters/m);
    });
  ...
  // NO client.ts — worksheet is a pure data module, ZERO React hooks anywhere.
  // NO "use client" directive anywhere in modules/accounting/worksheet/.
  // NO legacy WorksheetRepository class name in server barrel (old features/ class;
  // new canonical class is PrismaWorksheetRepo — encapsulated behind composition-root).
  // ───────────────────────────────────────────────────────────────────────────

  describe("Block 4 — REQ-002 NEGATIVE: D5 INVERSE (no client.ts, no use client, no legacy class)", () => {
    it("α64: NO presentation/client.ts file exists (D5 INVERSE — zero React hooks in worksheet)", () => {
      const clientTs = presentationFile("client.ts");
      // This file must NOT exist — worksheet has no client-side hook.
      // Consumers import TYPES only from barrel — no client.ts needed.
      expect(fs.existsSync(clientTs)).toBe(false);
    });

    it('α65: NO "use client" directive exists anywhere in modules/accounting/worksheet/**', () => {
    ...
    it("α66: server.ts does NOT export WorksheetRepository (legacy class name banned — encapsulated behind port)", () => {
      // WorksheetRepository was the old features/ class name (before PrismaWorksheetRepo rename).
      // server barrel must NOT re-export it — composition-root handles wiring,
      // route.ts post-C4 uses makeWorksheetService() factory instead of direct class.
      const content = readPresentationFile("server.ts");
      expect(content).not.toMatch(/WorksheetRepository\b/m);
    });
  });
});
