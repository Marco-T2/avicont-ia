...
const row = (override: Partial<AccountsPayable> = {}): AccountsPayable => ({
  id: "pay-1",
...
describe("payables mapper", () => {
  describe("toDomain()", () => {
    it("hydrates a Payable from a Prisma row", () => {
      const p = toDomain(row());
      expect(p.id).toBe("pay-1");
      expect(p.organizationId).toBe("org-1");
      expect(p.contactId).toBe("contact-1");
      expect(p.description).toBe("Factura proveedor");
      expect(p.amount.value).toBe(1000);
      expect(p.paid.value).toBe(250);
      expect(p.balance.value).toBe(750);
    ...
    it("converts Decimal monetary fields to plain numbers", () => {
      ...
      expect(p.paid.value).toBe(100.5);
      expect(p.balance.value).toBe(1134.06);
    });

    it("preserves PENDING status", () => {
      const p = toDomain(row({ status: "PENDING", paid: new Prisma.Decimal(0), balance: new Prisma.Decimal(1000) }));
