 ...
 * four shapes over `save()`'s dual write — AP create + D2 creation stamp with
 ...
 * persisted AP row where absence is asserted.
 */
...
describe("PrismaPayablesRepository — atomically() Postgres integration", () => {
  ...
  async function createLinkedFixture(): Promise<{ jeId: string; apId: string }> {
    ...
    const je = await prisma.journalEntry.create({
      data: {
        ...
        // Mirrors the linked AP row's dueDate below: the co-population CHECK
  ...
  async function createUnstampedFixture(): Promise<{ jeId: string }> {
    ...
    const je = await prisma.journalEntry.create({
      data: {
        ...
        number: jeNumber,
        date: new Date("2099-03-15T12:00:00Z"),
        description: "ppr-atom unstamped fixture journal",
        periodId: testPeriodId,
        voucherTypeId: testVoucherTypeId,
        createdById: testUserId,
  ...
  const SAVE_DUE = new Date("2099-04-20T12:00:00Z");

  /** Fresh PENDING entity linked to the unstamped JE — drives save()'s dual write. */
  function pendingEntity(jeId: string): Payable {
    return Payable.create({
      organizationId: testOrgId,
      contactId: testContactId,
      description: "ppr-atom save() fixture payable",
      amount: 100,
      dueDate: SAVE_DUE,
      journalEntryId: jeId,
    });
  ...
  it("root client commit: save() dual write lands atomically — AP creada + JE stamped {PENDING, entity.dueDate}", async () => {
    const { jeId } = await createUnstampedFixture();
    const entity = pendingEntity(jeId);

    await new PrismaPayablesRepository().save(entity);
    ...
    const jeRow = await prisma.journalEntry.findUnique({ where: { id: jeId } });
    expect(apRow!.status).toBe("PENDING");
    expect(apRow!.journalEntryId).toBe(jeId);
    // D2 creation stamp + P5 dueDate — ambos desde la entity, no hardcodeados.
    expect(jeRow!.paymentStatus).toBe("PENDING");
    expect(jeRow!.dueDate).toEqual(SAVE_DUE);
  });

  it("root client rollback: second write fails → AP create revertido y JE queda NULL/NULL", async () => {
    const { jeId } = await createUnstampedFixture();
    const entity = pendingEntity(jeId);
    const failingRepo = new PrismaPayablesRepository(
      withFailingSecondWrite(prisma) as unknown as RepoDb,
    ...
    // Sin el $transaction wrap, el create habría autocommiteado y la AP
    // existiría huérfana de stamp (drift silencioso H2, dirección save()).
    const apRow = await prisma.accountsPayable.findUnique({
      where: { id: entity.id },
    });
    const jeRow = await prisma.journalEntry.findUnique({ where: { id: jeId } });
    expect(apRow).toBeNull();
  ...
  it("nested savepoint rollback: forced failure rolls back save() WITHOUT killing the outer tx", async () => {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // El outer tx debe seguir vivo tras el rollback del savepoint interno —
      // la AP no existe, así que el write de prueba va sobre la JE fixture.
    ...
    const jeRow = await prisma.journalEntry.findUnique({ where: { id: jeId } });
    // Savepoint rollback: AP create revertido, JE sin stamp.
    expect(apRow).toBeNull();
    expect(jeRow!.paymentStatus).toBeNull();
    expect(jeRow!.dueDate).toBeNull();
    // Outer tx NO fue matado por el fallo interno.
