 ...
 * push INTO hex shape (paired-payables side, paired sister mirror receivables).
 *
 * Axis: functional move 3 privados shim INTO modules/payables/presentation/server.ts
 ...
 *
 * Marco lock final RED scope (4 assertions α paired-payables side — paired sister
 * mirror receivables 4 assertions = 8 paired total parsimonious vs C1a 10):
 ...
 * `PayableWithContact` consistente shim (pre) ↔ hex (post).
 *
 ...
 *     mapper aún en features/payables/payables.service.ts L133-135). Regex
 *     match falla.
 *   - T4 FAIL: server.ts NO contiene `prisma.contact.find` reference (no prisma
 *     import, query aún en features/payables/payables.service.ts L107-110 +
 ...
 * asserta path `modules/payables/presentation/server.ts` que persiste post C7
 ...
 *   - modules/payables/presentation/server.ts (target boundary hex — barrel re-export pre-RED 51 LOC, post-GREEN +functional)
 *   - modules/payables/presentation/composition-root.ts (factories pre-existing post C1a closure)
 *   - modules/receivables/presentation/__tests__/c1b-attach-contact-boundary-shape.paired-pr.test.ts (paired sister mirror RED this batch)
...
// ── C1b boundary hex target (1 archivo paired-payables side) ──────────────────
...
describe("POC paired payables↔receivables C1b — boundary attachContact Option A push INTO hex shape (paired-payables side, §13.A5-γ MATERIAL boundary preservation, 7ma evidencia §13.A5-α paired sister sub-cycle functional move NEW kind)", () => {
  // ── Hex boundary export POSITIVE (Tests 1-2) ────────────────────────────

  it("Test 1: modules/payables/presentation/server.ts exports attachContact (singular, getById/create/update-style hex boundary)", () => {
    const source = fs.readFileSync(PAYABLES_SERVER, "utf8");
    expect(source).toMatch(ATTACH_CONTACT_EXPORT_RE);
  });

  it("Test 2: modules/payables/presentation/server.ts exports attachContacts (plural, list-style hex boundary)", () => {
    const source = fs.readFileSync(PAYABLES_SERVER, "utf8");
  ...
  it("Test 3: modules/payables/presentation/server.ts contains Prisma.Decimal reference (mapper reconstruction MonetaryAmount.value→Prisma.Decimal at boundary hex per Marco lock L3)", () => {
    const source = fs.readFileSync(PAYABLES_SERVER, "utf8");
    expect(source).toMatch(PRISMA_DECIMAL_RE);
  });

  it("Test 4: modules/payables/presentation/server.ts contains prisma.contact.find reference (cross-module direct contact table query at boundary hex per Marco lock L4 F3-C — TODO ContactHydratePort extraction future POC contacts)", () => {
    const source = fs.readFileSync(PAYABLES_SERVER, "utf8");
