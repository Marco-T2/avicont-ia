 ...
 * Mirror sale C6 `PrismaSaleUnitOfWork` — delegates to `withAuditTx` — las 4
 * invariantes (correlationId pre-tx, SET LOCAL inside, fn invoke, return
 * shape) son inherited unchanged. Solo cambia el shape del scope: 5
 * superficies tx-bound + fiscalPeriods (BaseScope) + correlationId.
 *
 * IVA cascade ports (`ivaBookRegenNotifier`, `ivaBookVoidCascade`) + IVA
 * service/scope factories retired in lcv-feature-retirement (RND 102100000011
 * Dec-2021). Constructor reduced from 6 deps to 4 (mirrors sale UoW).
 *
...
export class PrismaPurchaseUnitOfWork implements PurchaseUnitOfWork {
  constructor(
     ...
     *  PrismaJournalEntryFactoryAdapter so generateForPurchase resolves the
     *  OperationalDocType FK (FL|PF|CG|SV) via findByCode. */
