...
function makeHarness() {
  // assertAuditContextSet uses $queryRaw to read app.current_user_id.
  // The mock returns a non-empty user_id so the runtime assertion passes —
  // these tests focus on tx-threading semantics, not the INV-1 guard itself.
  const externalTx = {
    __externalTx: true,
