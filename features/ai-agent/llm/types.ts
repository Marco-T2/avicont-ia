 ...
 *
 * Intentional omissions today (deliberate, not oversight):
 *   - history: not yet exposed; agent.service stitches history into the
 *     systemPrompt as text. Will move here when we adopt true multi-turn.
 *   - temperature / topP / maxTokens: will be added when we expose
 *     per-org tuning.
 *   - stream: will be added when the UI supports incremental rendering.
 *   - abortSignal: will be added when we wire request cancellation.
 */
