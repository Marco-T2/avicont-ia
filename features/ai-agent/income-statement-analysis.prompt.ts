...
//
// Coherencia conceptual con el JSON curado del Balance General:
// mismas formas para `groups → accounts`, mismo formato monetario boliviano,
// mismo bloque `currency`. Diferencias deliberadas: rango (no fecha
// puntual), 2 secciones (income/expenses), flag `hasOperatingLoss`,
// bloque `crossBalanceSheet` con los saldos del BG necesarios para los
// ratios cruzados, y `ratios` pre-calculados (single source of truth).
...
// Mismo umbral relativo (10%) que usa balance-sheet-analysis para clasificar
// un descuadre como estructural y bloquear el análisis antes de llamar al LLM.
...
export function checkIncomeStatementTriviality(
  ...
  // 1. Sin actividad financiera (ingresos y gastos en cero). Funcionalmente
  //    equivalente a "estado vacío" dado el filtrado del builder, que omite
  //    cuentas y grupos con balance cero.
  ...
  // 2. Sin ingresos pero con gastos: ratios sobre ingresos no son calculables.
  ...
  // 3. BG cruzado con descuadre estructural (>10% sobre el total de activo):
  //    los tres ratios cruzados no son confiables.
  if (bg.imbalanced) {
...
//
// A diferencia del Balance General (donde el LLM calcula los ratios), aquí
// el sistema los pre-calcula y los pasa al LLM como single source of truth.
// Razones: (a) determinismo aritmético; (b) auditabilidad ante el contador;
// (c) belt-and-suspenders análogo al pre-formato monetario.
...
function calculateRatios(
  ...
  // Carga Financiera = Gastos Financieros / Ingresos totales
  ...
  // No interpretable si patrimonio ≤ 0 (organización con patrimonio negativo).
  const roe =
  ...
  // Rotación de Activos = Ingresos totales / Activo Total (sin %)
