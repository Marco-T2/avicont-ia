...
// ── Serialized (JSON) variants ─────────────────────────────────────────────────
//
// Mirror of `SerializedEquityStatement` in equity-statement.types.ts: Decimals
// are emitted as strings to preserve precision across the JSON boundary.

export interface SerializedInitialBalanceRow {
  accountId: string;
  code: string;
  name: string;
  subtype: AccountSubtype;
...
export interface SerializedInitialBalanceGroup {
  subtype: AccountSubtype;
  label: string;
  rows: SerializedInitialBalanceRow[];
  subtotal: string;
}

export interface SerializedInitialBalanceSection {
  key: "ACTIVO" | "PASIVO_PATRIMONIO";
  label: string;
  groups: SerializedInitialBalanceGroup[];
  sectionTotal: string;
}

export interface SerializedInitialBalanceStatement {
  orgId: string;
  org: InitialBalanceOrgHeader;
  dateAt: string;
  sections: [SerializedInitialBalanceSection, SerializedInitialBalanceSection];
  imbalanced: boolean;
